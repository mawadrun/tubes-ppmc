...
int isValid_dijkstra(int x, int y, int rows, int cols) { // check boundary
...
int dijkstra(char maze[MAX][MAX], int rows, int cols, Point_dijkstra start, Point_dijkstra end) {
    ...
    while (heapSize > 0) { // extract node with minimum distance
        ...
        for (int k = 0; k < 4; k++) { // check neighbors
            int newX = u.x + dx[k];
            int newY = u.y + dy[k];
    ...
    if (dist[end.x][end.y] == INT_MAX) { // if end point can't be reached
