...
{
    if (currentPoint->x >= 0 && currentPoint->x < kolom && currentPoint->y >= 0 && currentPoint->y < baris && maze[currentPoint->y][currentPoint->x] != '#' && maze[currentPoint->y][currentPoint->x] != 'x')
    {
        return true;
    }
    else
...
void divideAndConquer(char maze[255][255], point *currentPoint, int baris, int kolom, point *end, int pathLength, FILE *fptr)
{
    ...
    { // kasus basis: koordinat point sama dengan koordinat end
        maze[end->y][end->x] = 'x';
